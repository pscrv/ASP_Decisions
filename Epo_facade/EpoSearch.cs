    ...
    public static class EpoSearch
    {
        ...
        public static void GetDecisionText(Decision decision, bool forceUpdate = false)
        {
            ...
            else
            {
                ...
                decision.Facts = HtmlEntity.DeEntitize(_stringFromParas(_loopToHeader(headers[0].ParentNode.NextSibling)));
                decision.ReasonsHeader = headers[1].InnerText;
                decision.Reasons = HtmlEntity.DeEntitize(_stringFromParas(_loopToHeader(headers[1].ParentNode.NextSibling)));
                decision.OrderHeader = headers[2].InnerText;
                decision.Order = HtmlEntity.DeEntitize(_stringFromParas(_loopToHeader(headers[2].ParentNode.NextSibling)));
                decision.TextDownloaded = true;
